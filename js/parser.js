...
class CFGParser {
  ...
  parseAll(input, strategy = 'leftmost') {
    ...
    // Increase parser limits for longer inputs so deep derivations still complete.
    this.maxDepth = Math.max(this.maxDepth, tokens.length * 2);
    this.maxStates = Math.max(this.maxStates, tokens.length * 2500);
    this.maxResults = Math.max(this.maxResults, 15);
    ...
    const MAX_VISITS_PER_SENTENTIAL = 15;
    ...
    while (queue.length > 0 && processed < this.maxStates && results.length < this.maxResults) {
      ...
      const sentKey = state.sentential.join(' ');
      const visitCount = visited.get(sentKey) || 0;
      if (visitCount >= MAX_VISITS_PER_SENTENTIAL) {
        continue;
      }
      visited.set(sentKey, visitCount + 1);
